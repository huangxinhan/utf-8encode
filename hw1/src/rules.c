...
void init_rules(void) {
    // To be implemented.
    main_rule = NULL;
    //now clear out the rule_map by setting everything in the rulemap to NULL
    int i; //for the loop
...
SYMBOL *new_rule(int v) {
    // To be implemented.
    //struct symbol newSymbol = {v, 0, NULL, NULL, NULL, NULL, NULL};

    struct symbol *ptr = NULL;
    ptr = new_symbol(v, ptr); //gets the pointer to the new symbol
    ptr->rule  = ptr; //set the rule to itself???
    ptr->next = ptr; //set the next to itself as well
    ptr->prev = ptr; //set the previous to itself as well

    //ptr->nextr = ptr; //set the prev and next rules to itself as well
    //ptr->prevr = ptr;

    debug("rule added with value %d", v);
    return ptr; //now return the pointer, which is also the head
}
...
void add_rule(SYMBOL *rule) {
    // To be implemented.
    debug("added rule with the value %d", rule->value);
    if (main_rule == NULL){
        //The previous rules and the next rules are all itself
        rule->prevr = rule;
        rule->nextr = rule;
        //(*rule).prevr = rule;
        //(*rule).nextr = rule;
        main_rule = rule; //specified rule becomes the main rule
    }
        ...
        //Rule is inserted at the very end
    }
...
void delete_rule(SYMBOL *rule) {
    ...
    if (rule->rule != NULL){ //if the rule to be removed is the head
        if(rule->refcnt == 0){
            //Then we recycle the head symbol
            recycle_symbol(rule);
            //Else nothing happens
        }
    }
    //recycle the rule or something idk man.
}
...
void unref_rule(SYMBOL *rule) {
    ...
    if (rule->refcnt < 1){
        //issues a message
        fprintf(stderr, "reference count will be negative");
        abort(); //then aborts
    }
    else{
        rule->refcnt--; //decrement the refcnt count otherwise
    }
}
